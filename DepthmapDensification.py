...
def propagation(is_even,
    ...
    d_2_deriv_y = 2.0 * mat[y][x] - mat[y - 1][x] - mat[y + 1][x]
    w_x = math.exp(-1.0 * abs(sobel_xx[y][x] / sigma_1))
    w_y = math.exp(-1.0 * abs(sobel_yy[y][x] / sigma_1))
    Ed_current = (depthMap[y][x] - mat[y][x]) ** 2 + \
        alpha * (w_x * (d_2_deriv_x * d_2_deriv_x)
                 + w_y * (d_2_deriv_y * d_2_deriv_y))

    # check right, down neighbor
    if is_even:
        # right neighbor(x+1, y) energy
        d_2_deriv_x = 2.0 * mat[y][x + 1] - mat[y][x] - mat[y][x + 2]
        d_2_deriv_y = 2.0 * mat[y][x + 1] - mat[y - 1][x + 1] - mat[y + 1][x + 1]
        w_x = math.exp(-1.0 * abs(sobel_xx[y][x + 1] / sigma_1))
        w_y = math.exp(-1.0 * abs(sobel_yy[y][x + 1] / sigma_1))
        Ed_right = (depthMap[y][x + 1] - mat[y][x + 1]) ** 2 + \
            alpha * (w_x * (d_2_deriv_x * d_2_deriv_x)
                     + w_y * (d_2_deriv_y * d_2_deriv_y))
        ...
        d_2_deriv_y = 2.0 * mat[y + 1][x] - mat[y][x] - mat[y + 2][x]
        w_x = math.exp(-1.0 * abs(sobel_xx[y + 1][x] / sigma_1))
        w_y = math.exp(-1.0 * abs(sobel_yy[y + 1][x] / sigma_1))
        Ed_down = (depthMap[y + 1][x] - mat[y + 1][x]) ** 2 + \
            alpha * (w_x * (d_2_deriv_x * d_2_deriv_x)
                     + w_y * (d_2_deriv_y * d_2_deriv_y))
...
def Test2(alpha=50.0, sigma_1=0.1, sigma_2=0.1, num_iter=50):
    print('init alpha: %.3f, sigma_1: %.3f, sigma_2: %.3f, num_iter: %d'
    ...
    for iter_i in range(1, num_iter + 1):
        ...
        # 输出此轮迭代能量函数值和alpha
        print('=> Iter %d | Ed_sum: %.3f, alpha: %.3f'
              % (iter_i, Ed_sum, alpha))
...
def Test(alpha=50.0, sigma_1=0.5, sigma_2=0.5, num_iter=60):
    print('Init alpha: %.3f, sigma_1: %.3f, sigma_2: %.3f, num_iter: %d'
          % (alpha, sigma_1, sigma_2, num_iter))
    ...
    for y in range(DEPTH_H):
        for x in range(DEPTH_W):
            ...
            if x - 1 < 0 or x + 1 >= DEPTH_W \
                ...
                w_x = math.exp(-1.0 * abs(sobel_xx[y][x] / sigma_1))
                w_y = math.exp(-1.0 * abs(sobel_yy[y][x] / sigma_2))
                Ed_sum += alpha * (w_x * (d_2_deriv_x * d_2_deriv_x)
                                   + w_y * (d_2_deriv_y * d_2_deriv_y))

    print('=> Iter_0 | Ed_sum: %.3f' % Ed_sum)
    ...
    for iter_i in range(num_iter):
        ...
        if is_even:
            for y in range(1, DEPTH_H - 2):
                for x in range(1, DEPTH_W - 2):
                    propagation(is_even,
                                depthMap,
                                sobel_xx, sobel_yy,
                                x, y,
            ...
            for y in range(DEPTH_H - 2, 1, -1):
                for x in range(DEPTH_W - 2, 1, -1):
                    propagation(is_even,
                                depthMap,
                                sobel_xx, sobel_yy,
                                x, y,
        ...
        mat2show = copy.deepcopy(mat)
        mat2show = mat2show.astype('uint8')
        cv2.imwrite('./iter_%d.jpg' % (iter_i + 1), mat2show)

    print('=> Final Ed_sum: ', Ed_sum)
    mat2show = copy.deepcopy(mat)
    mat2show = mat2show.astype('uint8')

    mat_ = cv2.resize(mat2show,
                      (int(mat2show.shape[1] * 0.3),
                       int(mat2show.shape[0] * 0.3)),
                      cv2.INTER_CUBIC)
