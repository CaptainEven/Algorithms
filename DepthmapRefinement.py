...
def Test(num_iter=50, eta=0.5, sigma_c=10, sigma_s=10, L=256):
    """
    ...
    src_path = './src4.jpg'
    depthMap_path = './depth4.jpg'  # Fusion的结果
    ...
    cost_volume = np.zeros((HEIGHT, WIDTH, 256), dtype='float32')
    cost_cw = np.zeros((HEIGHT, WIDTH, 256), dtype='float32')

    THRESH = eta * L
    for iter_i in range(num_iter):
        for d in range(256):  # depth range
            tmp = np.empty((HEIGHT, WIDTH))
            tmp.fill(d)
        ...
        for y in range(HEIGHT):
            for x in range(WIDTH):
                ...
                if int_depth + 1 > 255 or int_depth - 1 < 0:
                    ...
                    sub_depth = float(int_depth) - ((f_d_plus - f_d_minus) / (
        ...
        cv2.imwrite('./lamp_laplace_iter_%d.jpg' % (iter_i + 1), mat2show)
        print('=> iter %d done\n' % (iter_i + 1))
